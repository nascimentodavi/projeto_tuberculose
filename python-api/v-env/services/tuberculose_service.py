...
UF_MAP = {uf: regiao for regiao, ufs in REGIAO_MAP.items() for uf in ufs}
...
def casos_confirmados_tuberculose_regiao_estado_ano():
    ...
    df_long = pd.melt(df_wide, id_vars=['localidade'], var_name='ano', value_name='casos')

    # Converte a coluna 'ano' para tipo numérico, tratando possíveis erros.
    df_long['ano'] = pd.to_numeric(df_long['ano'], errors='coerce')
    df_long.dropna(subset=['ano'], inplace=True) # Remove linhas onde a conversão falhou
